...
type ContractInfo struct {
	...
	OpcodeList   []string       // 调用的 opcode 列表
}
...
// 实例化一个Code栈该栈保存了运行过的 opcode
// var CodeStack = Stack{ptr: 0}
...
func TransactionInfoHook(name string, data interface{}) {
	...
	case "TxHash":
		if data, ok := data.(common.Hash); ok {
			...
			contractInfo = ContractInfo{} //初始化 ContractInfos
			txInfo.TxHash = data
		} else {
...
func ContractInfoHook(name string, data interface{}) {
	...
	case "ContractAddr":
		if data, ok := data.(common.Address); ok {
			...
			contractInfo = ContractInfo{ContractAddr: data}
			...
			print("👎[TransactionInfoHook] ContractHash not match common.Hash type")
		}
	case "Opcode":
		if data, ok := data.(string); ok {
			contractInfo.OpcodeList = append(contractInfo.OpcodeList, data)
		} else {
			print("👎[TransactionInfoHook] Opcode not match byte type")
		}
	}
}
...
func AfterRun() {
	...
	if !AddrStack.isEmpty() {
		contractInfo = ContractInfo{ContractAddr: AddrStack.top().(common.Address), Layer: AddrStack.size()} //新建一个contract Info实例
	}
}
